...
class MLP_MNIST_Classifier(pl.LightningModule): 
    ...
    def __init__(self, 
                 ...
                 ## ----------> 
                 #하이퍼 파라미터라고 가정 하고 저장할 수 있도록 함
                 learning_rate=1e-3
                 ## <---------                 
                 ):
        super().__init__()
        self.save_hyperparameters()  # <-- it store arguments to self.hparams.* 
        #self.hparams.learning_rate #다음 처럼 접근 가능
...
def cli_main():
    pl.seed_everything(1234) #랜덤 시드 세팅 : 파이썬, 넘파이, 토치, 쿠다
    ...
    parser = MLP_MNIST_Classifier.add_model_specific_args(parser) #모델의 특정 하이퍼파라미터 기본 값 넣기
